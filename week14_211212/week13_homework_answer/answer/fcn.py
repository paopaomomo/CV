...
class FCN32s(nn.Module):
    ...
    def __init__(self, pretrained_net, n_class):
        ...
        # output = (N-1)*2+1-2+3=2N-2+1-2+3 = 2N 
        # 正：output = (input -k +2p)/s + 1 : output = input/2
        # output = (input-3+2)/2+1 = input/2-0.5+1= floor(input/2 + 0.5)= input/2
        # (input-3+2)%2
        # input = (output -1 )s-2p +k + (input-3+2)%2
        # input = (output - 1)s-2p + k + output_padding
        # output  = (I-2)/2+1 = I/2 
