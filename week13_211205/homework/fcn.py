...
class FCN8s(nn.Module):
    ...
    def forward(self, x):
        ...
        #[homework] 实现跳级结构
        score = self.  # size=(N, 512, x.H/16, x.W/16)
        score = self.  # element-wise add, size=(N, 512, x.H/16, x.W/16)
        score = self.  # size=(N, 256, x.H/8, x.W/8)
        score = self.  # element-wise add, size=(N, 256, x.H/8, x.W/8)
        score = self.  # size=(N, 128, x.H/4, x.W/4)
        score = self.  # size=(N, 64, x.H/2, x.W/2)
        score = self.  # size=(N, 32, x.H, x.W)
        score = self.  # size=(N, n_class, x.H/1, x.W/1)
